...
> `Pytorch/Caffe` 框架输入输出数据的尺寸都是 （`(N, C, H, W)`），常规卷积的卷积核 shape 都为（`C_out, C_in, kernel_height, kernel_width`），常规卷积是这样，分组卷积的卷积核 `shape` 为（`C_out, C_in/g, kernel_height, kernel_width`）和 `DW` 卷积的卷积核 `shape` 为（`C_in, 1, kernel_height, kernel_width`）。
